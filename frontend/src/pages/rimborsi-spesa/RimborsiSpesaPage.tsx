...
export function RimborsiSpesaPage() {
  ...
  const [soloMiei, setSoloMiei] = useState(!isPrivilegiato && !isAmministrativo);
  ...
  const { data: progetti } = useQuery({
    queryKey: queryKeys.progetti.list({
      ...
      solo_allocati: user?.ruolo !== 'amministrativo' && user?.ruolo !== 'superadmin' && user?.ruolo !== 'direttore_generale' && user?.ruolo !== 'monitor' ? true : undefined,
    }),
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['rimborsi-spesa', stato, progettoId, soloMiei, page],
    queryFn: () => rimborsiSpesaApi.list({ stato, progetto_id: progettoId, solo_miei: soloMiei, page, page_size: 20 }).then(r => r.data),
  });
